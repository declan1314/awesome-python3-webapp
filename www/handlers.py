...
from ssh_operate import download_file, get_folders_and_files
...
def common_get_path(request, server_id, path_id, path=''):
    ...
    back_url = '/servers/' + server_id + '/paths' + ('/' + path_id if not len(path) == 0 else '') + (
        '/path/' + parse.quote(path[:path.rfind('/')], 'utf-8') if not len(path) == 0 and path.rfind('/') != -1 else '')
    return {
